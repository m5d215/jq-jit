...
- 新しい raw-byte fast path を足す時は、**非期待型の入力でも正しく error を raise する**ように書くか、そうでないなら入力型を先頭バイトで判別して bail。
- 検出: `diff ループ` で `f` と `(f)?` の両方を jq と突き合わせる。`(f)?` だけ divergence したら null-masking。

    ...
    if null_input {
        ...
        if raw_input {
            ...
            if slurp {
                ...
                let parse_result = if filter.is_empty() {
                ...
                } else if let Some((ref srj_field, ref srj_split, ref srj_join)) = field_split_rev_join {
                    // .field | split("x") | reverse | join("y") — split, reverse parts, join
                    let split_bytes = srj_split.as_bytes();
                    let join_escaped = json_escape_bytes(srj_join.as_bytes());
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if let Some((vs, ve)) = json_object_get_field_raw(raw, 0, srj_field) {
                            let val = &raw[vs..ve];
                            if val.len() >= 2 && val[0] == b'"' && val[val.len()-1] == b'"' {
                                let inner = &val[1..val.len()-1];
                                if !inner.contains(&b'\\') {
                                    // Split on sep, collect parts
                                    ...
                                    if sep.len() == 1 {
                                        ...
                                        while last <= il {
                                            if last + sl <= il {
                                                if let Some(pos) = inner[last..].windows(sl).position(|w| w == sep) {
                                                    parts.push(&inner[last..last + pos]);
                                                    last = last + pos + sl;
                                                } else {
                                                    parts.push(&inner[last..]);
                                                    break;
                ...
                } else if let Some((ref ssj_field, ref ssj_split, ssj_from, ssj_to, ref ssj_join)) = field_split_slice_join {
                    // .field | split(sep) | .[from:to] | join(sep2) — raw byte
                    let split_bytes = ssj_split.as_bytes();
                    let single_split = if split_bytes.len() == 1 { Some(split_bytes[0]) } else { None };
                    let escaped_join = json_escape_bytes(ssj_join.as_bytes());
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if let Some((vs, ve)) = json_object_get_field_raw(raw, 0, ssj_field) {
                            let val = &raw[vs..ve];
                            if val.len() >= 2 && val[0] == b'"' && val[val.len()-1] == b'"'
                                && memchr::memchr(b'\\', &val[1..val.len()-1]).is_none()
                                && !split_bytes.is_empty()
                            {
                                ...
                                let from = match ssj_from {
                                    Some(f) => if f < 0 { (len + f).max(0) as usize } else { (f as usize).min(len as usize) },
                                    None => 0,
                                };
                                let to = match ssj_to {
                                    Some(t) => if t < 0 { (len + t).max(0) as usize } else { (t as usize).min(len as usize) },
                                    None => len as usize,
                                };
                                compact_buf.push(b'"');
                                if from < to && from < segments.len() {
                                    let end_idx = to.min(segments.len());
                                    for (i, seg) in segments[from..end_idx].iter().enumerate() {
                                        if i > 0 { compact_buf.extend_from_slice(&escaped_join); }
                ...
                } else if let Some((ref slc_field, slc_op, slc_n)) = select_length_cmp {
                    // select(.field | length cmp N) — output whole object
                    use jq_jit::ir::BinOp;
                    let threshold = slc_n as usize;
                    let threshold_f = slc_n;
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if let Some((vs, ve)) = json_object_get_field_raw(raw, 0, slc_field) {
                            let cond_val = &raw[vs..ve];
                            let pass = if cond_val.len() >= 2 && cond_val[0] == b'"' && cond_val[cond_val.len()-1] == b'"'
                ...
                } else if let Some((ref ct_field, ct_upper, ref ct_pattern)) = field_case_test {
                    // .field | ascii_case | test("regex") — case convert + regex test → true/false
                ...
                } else if let Some((ref conj, ref cmps, ref rexpr)) = select_compound_computed {
                    use jq_jit::ir::BinOp;
                    let is_and = matches!(conj, BinOp::And);
                    // Collect all needed fields
                    let mut all_fields: Vec<String> = Vec::new();
                ...
                } else if let Some((ref conj, ref cmps, ref sac_parts)) = select_compound_str_chain {
                    // select(.x > N and .y < M) | str_add_chain
                    use jq_jit::interpreter::StringAddPart;
                    use jq_jit::ir::BinOp;
                    let mut field_names: Vec<&str> = Vec::new();
                    let mut field_idx_map: std::collections::HashMap<&str, usize> = std::collections::HashMap::new();
                    // Add condition fields
                    for (f, _, _) in cmps.iter() {
                        if !field_idx_map.contains_key(f.as_str()) {
                            field_idx_map.insert(f.as_str(), field_names.len());
                            field_names.push(f.as_str());
                        }
                    }
                    // Add chain fields
                    for part in sac_parts.iter() {
                    ...
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if json_object_get_fields_raw_buf(raw, 0, &field_names, &mut ranges_buf) {
                            // Evaluate compound condition: get nums for condition fields
                            let pass = if is_and {
                                resolved_cmps.iter().all(|&(idx, op, threshold)| {
                                    let (vs, ve) = ranges_buf[idx];
                                    let s = unsafe { std::str::from_utf8_unchecked(&raw[vs..ve]) };
                                    if let Ok(val) = s.trim().parse::<f64>() {
                                        match op { BinOp::Gt => val > threshold, BinOp::Lt => val < threshold, BinOp::Ge => val >= threshold, BinOp::Le => val <= threshold, BinOp::Eq => val == threshold, BinOp::Ne => val != threshold, _ => false }
                                    } else { false }
                                })
                            } else {
                                resolved_cmps.iter().any(|&(idx, op, threshold)| {
                                    let (vs, ve) = ranges_buf[idx];
                                    let s = unsafe { std::str::from_utf8_unchecked(&raw[vs..ve]) };
                                    if let Ok(val) = s.trim().parse::<f64>() {
                                        match op { BinOp::Gt => val > threshold, BinOp::Lt => val < threshold, BinOp::Ge => val >= threshold, BinOp::Le => val <= threshold, BinOp::Eq => val == threshold, BinOp::Ne => val != threshold, _ => false }
                ...
                } else if let Some((ref sel_field, ref sel_op, threshold, ref dk_field, ref dk_rexpr)) = select_cmp_dynkey {
                    // select(.field > N) | {(.dynkey): rexpr}
                    use jq_jit::ir::BinOp;
                    let mut all_fields: Vec<String> = Vec::new();
                    let mut field_idx = std::collections::HashMap::new();
                    field_idx.insert(sel_field.clone(), 0);
                    all_fields.push(sel_field.clone());
                    if !field_idx.contains_key(dk_field) {
                        field_idx.insert(dk_field.clone(), all_fields.len());
                        all_fields.push(dk_field.clone());
                    }
                    for name in remap_expr_fields(dk_rexpr) {
                        if !field_idx.contains_key(name) {
                            field_idx.insert(name.to_string(), all_fields.len());
                            all_fields.push(name.to_string());
                        }
                    ...
                    let field_refs: Vec<&str> = all_fields.iter().map(|s| s.as_str()).collect();
                    let dk_idx = field_idx[dk_field];
                    let resolved_val = resolve_one_remap(dk_rexpr, &field_idx);
                    let mut ranges_buf = vec![(0usize, 0usize); field_refs.len()];
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if let Some(val) = json_object_get_num(raw, 0, sel_field) {
                            ...
                            if pass {
                                if json_object_get_fields_raw_buf(raw, 0, &field_refs, &mut ranges_buf) {
                                    let (ks, ke) = ranges_buf[dk_idx];
                                    let key_raw = &raw[ks..ke];
                                    // key_raw is a JSON string like "user_123", use as-is for key
                                    compact_buf.push(b'{');
                ...
                } else if let Some((ref sel_field, ref sel_op, threshold, ref dk_field, ref dk_rexpr, ref static_pairs)) = select_cmp_dynkey_mixed {
                    // select(.field > N) | {(.dynkey): rexpr, static_key: rexpr2, ...}
                    use jq_jit::ir::BinOp;
                    let mut all_fields: Vec<String> = Vec::new();
                    let mut field_idx = std::collections::HashMap::new();
                    field_idx.insert(sel_field.clone(), 0);
                    ...
                    let field_refs: Vec<&str> = all_fields.iter().map(|s| s.as_str()).collect();
                    let dk_idx = field_idx[dk_field];
                    let resolved_dk_val = resolve_one_remap(dk_rexpr, &field_idx);
                    let resolved_static = resolve_remap_exprs(static_pairs, &field_idx);
                    // Pre-build static key prefixes: ,"key":
                    let static_key_prefixes: Vec<Vec<u8>> = static_pairs.iter().map(|(k, _)| {
                        let mut buf = Vec::with_capacity(k.len() + 5);
                        buf.extend_from_slice(b",\"");
                        buf.extend_from_slice(k.as_bytes());
                        buf.extend_from_slice(b"\":");
                        buf
                ...
                } else if let Some((ref sel_field, ref sel_op, threshold, ref sac_parts)) = select_cmp_str_chain {
                    // select(.field op N) | str_add_chain — select + string concat
                    use jq_jit::interpreter::StringAddPart;
                    use jq_jit::ir::BinOp;
                    // Collect unique field names (select field + chain fields)
                    let mut field_names: Vec<&str> = Vec::new();
                    let mut field_idx_map: std::collections::HashMap<&str, usize> = std::collections::HashMap::new();
                    // Ensure select field is in list
                    ...
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        // First check select condition via numeric field extraction
                        if let Some(val) = json_object_get_num(raw, 0, sel_field) {
                            let pass = match sel_op {
                                BinOp::Gt => val > threshold, BinOp::Lt => val < threshold,
                                BinOp::Ge => val >= threshold, BinOp::Le => val <= threshold,
                ...
                } else if let Some((ref kj_sep, kj_sorted)) = keys_join {
                    let sep_bytes = kj_sep.as_bytes();
                    let mut keys_buf = Vec::new();
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if !json_object_keys_join_to_buf(raw, 0, sep_bytes, kj_sorted, &mut compact_buf, &mut keys_buf) {
                            let v = json_to_value(unsafe { std::str::from_utf8_unchecked(raw) })?;
                            process_input(&v, None, &mut out, &mut compact_buf, &mut any_output_false, &mut had_error);
                        }
                        if compact_buf.len() >= 1 << 17 {
                ...
                } else if let Some((ref sm_field, sm_op, sm_thr, ref sm_pairs)) = select_cmp_merge {
                    // select(.field cmp N) | .+{key: literal, ...} — filter then merge
                    let merge_pairs: Vec<(String, Vec<u8>)> = sm_pairs.clone();
                    let mut tmp = Vec::new();
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if let Some(val) = json_object_get_num(raw, 0, sm_field) {
                            let pass = match sm_op {
                                jq_jit::ir::BinOp::Gt => val > sm_thr, jq_jit::ir::BinOp::Lt => val < sm_thr,
                                jq_jit::ir::BinOp::Ge => val >= sm_thr, jq_jit::ir::BinOp::Le => val <= sm_thr,
                                jq_jit::ir::BinOp::Eq => val == sm_thr, jq_jit::ir::BinOp::Ne => val != sm_thr,
                                _ => false,
                            };
                            if pass {
                                if use_pretty_buf {
                ...
                } else if let Some((ref ssm_field, ref ssm_op, ref ssm_arg, ref ssm_pairs)) = select_str_merge {
                    // select(.field | startswith/test("str")) | .+{key: literal, ...}
                    let merge_pairs: Vec<(String, Vec<u8>)> = ssm_pairs.clone();
                    let cond_re: Option<regex::Regex> = if ssm_op == "test" { regex::Regex::new(ssm_arg).ok() } else { None };
                    let arg_bytes = ssm_arg.as_bytes();
                    let mut tmp = Vec::new();
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if let Some((vs, ve)) = json_object_get_field_raw(raw, 0, ssm_field) {
                            let val = &raw[vs..ve];
                            if val.len() >= 2 && val[0] == b'"' && val[val.len()-1] == b'"' {
                                let inner = &val[1..val.len()-1];
                                let pass = match ssm_op.as_str() {
                                    "startswith" => inner.starts_with(arg_bytes),
                                    "endswith" => inner.ends_with(arg_bytes),
                ...
                } else if let Some(ref dfs) = del_fields {
                    let df_refs: Vec<&str> = dfs.iter().map(|s| s.as_str()).collect();
                    let mut tmp = Vec::new();
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        if use_pretty_buf {
                ...
                } else if let Some((ref cmp_op2, cmp_n2)) = count_each_select_cmp {
                    // [.[] | select(. cmp N)] | length — count matching values
                    use jq_jit::ir::BinOp;
                    let op = *cmp_op2;
                    json_stream_raw(&input_str, |start, end| {
                        let raw = &input_bytes[start..end];
                        ...
                        if ok {
                            ...
                            compact_buf.extend_from_slice(itoa_buf.format(count).as_bytes());
                            compact_buf.push(b'\n');
                        } else {
                            let v = json_to_value(unsafe { std::str::from_utf8_unchecked(raw) })?;
                            process_input(&v, None, &mut out, &mut compact_buf, &mut any_output_false, &mut had_error);
        ...
        for file in &files {
            ...
            let parse_result = if slurp {
                ...
                // select(.field | length cmp N) — stdin path, output whole object
                use jq_jit::ir::BinOp;
                let content_bytes = content.as_bytes();
                let threshold = slc_n as usize;
                let threshold_f = slc_n;
                ...
                if let Ok(re) = regex::Regex::new(&re_pattern) {
                    ...
                    let mut all_fields: Vec<String> = Vec::new();
                    let mut field_idx = std::collections::HashMap::new();
                    let ensure_field4 = |f: &str, all: &mut Vec<String>, idx: &mut std::collections::HashMap<String, usize>| {
                        if !idx.contains_key(f) { idx.insert(f.to_string(), all.len()); all.push(f.to_string()); }
                    };
                ...
                for (f, _, _) in cmps { ensure_field3(f.as_str(), &mut all_fields, &mut field_idx); }
                for f in remap_expr_fields(rexpr) { ensure_field3(f, &mut all_fields, &mut field_idx); }
                let resolved = resolve_one_remap(rexpr, &field_idx);
                let field_refs: Vec<&str> = all_fields.iter().map(|s| s.as_str()).collect();
                let cmp_indices: Vec<(usize, BinOp, f64)> = cmps.iter().map(|(f, op, thr)| {
                    (field_idx[f.as_str()], *op, *thr)
                ...
                // select(.x > N and .y < M) | str_add_chain — stdin path
                use jq_jit::interpreter::StringAddPart;
                use jq_jit::ir::BinOp;
                let content_bytes = content.as_bytes();
                let mut field_names: Vec<&str> = Vec::new();
                ...
                // select(.field > N) | {(.dynkey): rexpr} — stdin path
                use jq_jit::ir::BinOp;
                let content_bytes = content.as_bytes();
                let mut all_fields: Vec<String> = Vec::new();
                let mut field_idx = std::collections::HashMap::new();
                ...
                // select(.field > N) | {(.dynkey): rexpr, static_key: rexpr2, ...} — stdin path
                use jq_jit::ir::BinOp;
                let content_bytes = content.as_bytes();
                let mut all_fields: Vec<String> = Vec::new();
                let mut field_idx = std::collections::HashMap::new();
                ...
                // select(.field op N) | str_add_chain — stdin path
                use jq_jit::interpreter::StringAddPart;
                use jq_jit::ir::BinOp;
                let content_bytes = content.as_bytes();
                let mut field_names: Vec<&str> = Vec::new();
                ...
                // .field | ascii_case | test("regex") — case convert + regex test (stdin)
                if let Ok(re) = regex::Regex::new(ct_pattern) {
                    let content_bytes = content.as_bytes();
                    json_stream_raw(content, |start, end| {
                        let raw = &content_bytes[start..end];
                ...
                // [.[] | select(. cmp N)] | length — count matching values (stdin)
                use jq_jit::ir::BinOp;
                let content_bytes = content.as_bytes();
                let op = *cmp_op2;
                json_stream_raw(content, |start, end| {

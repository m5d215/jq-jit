...
pub fn eval(
    ...
    match expr {
        ...
        Expr::CallBuiltin { op, args } => {
            eval_call_builtin(*op, args, input, env, cb)
        }
...
fn eval_call_builtin(op: BuiltinOp, args: &[Expr], input: Value, env: &EnvRef, cb: &mut dyn FnMut(Value) -> GenResult) -> GenResult {
    // Special handling for builtins that take filter/closure arguments
    match (op, args.len()) {
        (BuiltinOp::InputLineNumber, 0) => {
            return cb(Value::number(get_input_line_number() as f64));
        }
        (BuiltinOp::InputFilename, 0) => {
            return cb(get_input_filename());
        }
        (BuiltinOp::GetSearchList, 0) => {
        ...
        (BuiltinOp::FromCsv, 0) | (BuiltinOp::FromTsv, 0) => {
            return eval_fromcsv(&input, op == BuiltinOp::FromTsv, cb);
        }
        (BuiltinOp::ToStream, 0) => {
            return eval_tostream(&input, cb);
        }
        (BuiltinOp::FromStream, 1) => {
            return eval_fromstream(&args[0], input, env, cb);
        }
        (BuiltinOp::TruncateStream, 1) => {
            return eval_truncate_stream(&args[0], input, env, cb);
        }
        (BuiltinOp::FromCsvh, _) | (BuiltinOp::FromTsvh, _) => {
            let is_tsv = op == BuiltinOp::FromTsvh;
            if args.is_empty() {
        ...
        (BuiltinOp::Combinations, 0) => {
            return eval_combinations(&input, cb);
        }
        (BuiltinOp::Combinations, 1) => {
    ...
    eval_call_builtin_args(op, args, args.len(), collected, input, env, cb)
}
...
fn eval_call_builtin_args(op: BuiltinOp, args: &[Expr], remaining: usize, collected: Vec<Value>, input: Value, env: &EnvRef, cb: &mut dyn FnMut(Value) -> GenResult) -> GenResult {
    ...
    eval(&args[idx], input.clone(), env, &mut |val| {
        ...
        next[idx + 1] = val;
        eval_call_builtin_args(op, args, remaining - 1, next, input.clone(), env, cb)
    })
}
